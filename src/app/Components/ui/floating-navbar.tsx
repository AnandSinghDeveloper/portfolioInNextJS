...
export const FloatingNav = ({
  ...
  return (
    <AnimatePresence mode="wait">
      ...
      >
        <div>
          <Link href="/" className="text-xl lg:text-4xl leading-none font-bold">
            <span className="text-[#5918DF] text-xl lg:text-3xl">{`<`}</span>
            Anand
          </Link>
          <span className="text-lg lg:text-3xl text-[#5918DF]">{`/>`}</span>
        </div>
        <div className="flex">
          {navItems.map((navItem, idx: number) => (
            ...
            >
              ...
              <span className="opacity-50 text-sm lg:text-xl font-light">{`/>`}</span>
            </a>
          ))}
        </div>
      </motion.div>
